import { mockAgents, mockTasks, mockActivities, mockUsageData } from "@/data/mockData";
import { BarChart3, Bot, CheckSquare, Activity, DollarSign, Zap } from "lucide-react";
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-4 gap-4">
        ...
        <KpiCard icon={<DollarSign size={20} />} label="Today's Cost" value="$0.73" color="success" />
        <KpiCard icon={<Zap size={20} />} label="Total Tokens" value="1.9M" color="warning" />
      </div>
          ...
          {mockActivities.slice(0, 5).map((act) => {
            ...
            return (
              <div key={act.id} className="flex items-center gap-3 px-4 py-3">
                ...
                {act.cost > 0 && (
                  <span className="text-xs font-medium text-muted-foreground">${act.cost.toFixed(2)}</span>
                )}
              </div>
            );
          })}
